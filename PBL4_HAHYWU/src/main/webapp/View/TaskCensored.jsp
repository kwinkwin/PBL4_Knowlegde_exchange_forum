...
<head>
    ...
    <script>
	    ...
	    function openPopup() {
	        // Code để mở popup
	        sessionStorage.setItem('popupStateCensored', 'open');
	    }
	    // Khi đóng popup, cập nhật trạng thái
	    function closePopup() {
	        // Code để đóng popup
	        sessionStorage.setItem('popupStateCensored', 'closed');
	    }
	    // Khi trang được load hoặc reload, kiểm tra trạng thái từ session storage
	    window.onload = function() {
	        var popupState = sessionStorage.getItem('popupStateCensored');
	        if (popupState === 'open') {
        ...
        function selectchoice() {
            var l = "GrabServlet?Censored=1&IDField=" + document.getElementById("mySelect").value + "&search=" + document.getElementById("txtsearch").value
    ...
    <main>
        <div class="pure-g taskbar">
            ...
            <div class="pure-u-10-24 taskbarleft">
                ...
                <a href="GrabServlet?Censored=1&idacc=<%= user.getID_Account() %>"><input class="taskbarchoice" type="submit" value="Censored"></a>
